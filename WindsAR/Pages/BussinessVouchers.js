...
const BussinessVouchers = ({navigation}) => {
  ...
  const validateInput = async () => {
    ...
    if (temp.isName && temp.isEmail) {
      const user_id = await AsyncStorage.getItem('business_id');
      let response = await fetch(
        'https://windsar.herokuapp.com/registerCustomer/businessEditProfile/',
        {
          method: 'POST',
          headers: {
      ...
      if (recievedReponse.success == 'True') {
        ...
        await AsyncStorage.setItem('business_name', name);
